...
function classTest2(){
  ...
  const inputList=document.getElementsByClassName("cls-test2");
    /* inputList는 요소가 아닌 배열 형태로 반환 된다! */
    /* 중요: 요소를 한번에 묶음으로 얻어오기 때문에 
    -> 요소를 하나씩 다루고 싶으면 index를 이용해서 
...
function nameTest(){
 ...
 //name속성값이 "hobby"인 요소를 모두 얻어와 hobbyList에 저장
...
function cssTest(){
 ...
 [요소 1개]
  1) document.querySelector("CSS 선택자");
            ...
            //"CSS 선택자"와 일치하는 요소가 있니? 있으면 선택해줘
  ...
  */ 
   ...
   container.style.border = "10px solid red";
    ...
    // 첫 번째 자식 div 선택 방법 2
    //->querySelector()의 첫 번째 요소만 선택한다는 특징 활용
...
function readValue(){
    ...
    if(input.value.trim().length==0){
        ...
        return;// 1) 현재 수행중인 함수를 종료 2) 호출한 곳으로 돌아감
    }
...
/* 아이디가 user- input인 요소에서 키가
올라오는 동작이 발생했을 때 (감지 되었을 때 )
