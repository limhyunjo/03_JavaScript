...
function compareFn2(){
  ...
  /* 콘솔에서 string으로 표시되는 문자열을 숫자로 바꿔줌 */
    
    
    // const v3=cv3.value;/* 이거로 하면 콘솔에서 숫자가 아닌 string으로 뜸 */
    // const v4=cv4.value;/* 이부분 집가서 영상 다시 보기 */
...
function checkFn3(){
    ...
    /* 백틱(`)을 이용한 문자열 작성하기 -js에서 제공하는 기능 */

    //--문자열 전체를 백틱으로 감싼 후 변수, 연산 결과 등이 출력되는 자리에 
    //${변수명}또는 ${연산식}을 작성
   
    // result3.innerText="5은/는 2의 배수 : false";
    result3.innerText=`${v1}은/는 ${v2}의 배수 : ${v1 % v2 == 0}`;
    /* 위는 실무에서 많이 쓰는 것 중요 */
...
function checkFn5(){
    ...
    const bool3 =  1<=3 && 3<=10;/* 이렇게 써도 됨 */
  console.log(`3은 1 부터 10 사이의 수가 맞는가? ${bool3}`);/* 영상 복습 */
...
//   (bool6 !=true) 는 true 이지만 앞에!가 붙어서 false
