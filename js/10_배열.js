...
//배열의 index(색인, 번호)
  ...
  */
    ...
    arr2[1]= "눈온다"
    arr2[2]=true;
    //(JS배열의 특징:인덱스별로 자료형을 다르게 할 수 있다)
    ...
    //1.길이 제한이 없다->배열에 원하는 만큼 값을 추가 가능
    arr1[0]="가";
...
function check3(){
    ...
    //->순차 접근 또는 반복 접근이라고 함
  const arr=[10,20,50,100,500,1000];//length는 6
   // index=0보다 큰 정수로 이루어짐
   // (i <= arr.length - 1)    ==    (i < arr.length)
  for(let i=0 ; i<=arr.length; i++){//0 부터 5까지 1씩 증가
...
function check4(){
    ...
    const arr=new Array(5);//5칸 짜리 (요소 별로 내용이 없는) 빈 배열
  ...
  let sum=0;// 합계
    ...
    /* 나중에 배열 길이가 바뀔 수 있어서 직접적인 숫자가 아닌 
       배열 길이로 씀 */
...
function check6(){
 ...
 for(let row=0; row<5; row++){//헹을 제어하는 for문
  
    //1차원 배열을 만들어 arr[i]에 대압
    arr[row]=[];
